# 실제 서비스에 들어갈 "음성 파일 -> Speech API 호출 -> 텍스트 추출 -> OpenAI 판정"
# 엔드투엔드 연동 스크립트.
# 이전(stt_pipeline_test.py)까지는 Speech Studio에서 수동으로 받은 JSON을 썼다면,
# 이건 코드가 직접 Speech API를 호출해서 그 JSON을 실시간으로 받아옵니다.
...
def transcribe_audio_file(audio_path: str) -> list:
    # Speech API를 실제로 호출해서, 오디오 파일 하나를 텍스트(Lexical, 마스킹 없음) 리스트로 변환하는 함수.
    # Speech Studio에서 다운받던 JSON을, 이제 코드가 직접 받아오는 것과 같은 역할.
    ...
    # "Detailed" 형식으로 요청해야 Lexical(마스킹 안 된 원문) 등 상세 정보를 받을 수 있음
    speech_config.output_format = speechsdk.OutputFormat.Detailed
    # 기본값이 욕설 마스킹(Masked)이라, 명시적으로 Raw로 지정해서 원문 그대로 받도록 설정
    ...
    def handle_result(evt):
        # 인식된 구간 하나가 끝날 때마다 호출됨
        if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
            detail = json.loads(evt.result.json)  # Speech Studio에서 보던 것과 동일한 JSON 구조
            nbest = detail.get("NBest", [])
            if nbest:
                lexical = nbest[0].get("Lexical", "").strip()
                if len(lexical) >= 2:  # 의미 없는 1글자 감탄사는 제외
...
if __name__ == "__main__":
    # ↓↓↓ 테스트할 음성 파일 경로를 넣으세요 (.wav 형식 권장) ↓↓↓
    AUDIO_FILE_PATH = "test2.wav"

    print(f"[1/2] '{AUDIO_FILE_PATH}' 파일을 Speech API로 변환 중...")
    segments = transcribe_audio_file(AUDIO_FILE_PATH)
    print(f"      → {len(segments)}개 발화 추출됨\n")

    print("[2/2] OpenAI로 유해발언 판정 중...\n")
